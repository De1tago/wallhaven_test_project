...
При запуске приложение само определяет окружение: на GNOME и большинстве Linux-окружений используется GTK, на KDE Plasma, Windows и в прочих ОС — Qt. В сборках (Flatpak, Linux-бинарник) доступны **оба** интерфейса, поэтому выбор происходит автоматически по текущему DE. Принудительно выбрать интерфейс можно флагом:
