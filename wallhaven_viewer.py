...
class MainWindow(Gtk.ApplicationWindow):
    ...
    # === ИЗМЕНЕННАЯ ЛОГИКА: Принимает кнопку-заглушку (placeholder_btn) ===
        ...
        def worker():
            ...
            if img_data is None:
                try:
                    ...
                    # В случае ошибки можно убрать спиннер или удалить кнопку
                    GLib.idle_add(lambda: self.flowbox.remove(placeholder_btn))
                    return 

            # 3. Обновление UI (передаем ту же кнопку)
            if img_data:
                try:
                    pixbuf = self.load_pixbuf_from_bytes(img_data)
                    if pixbuf:
                        GLib.idle_add(self.update_thumbnail_ui, placeholder_btn, pixbuf)
                except Exception as e:
                    print(f"Ошибка: {e}")

        threading.Thread(target=worker, daemon=True).start()
    
    # === ОБНОВЛЕНИЕ КНОПКИ (Вместо создания новой) ===
    def update_thumbnail_ui(self, btn, pixbuf):
        try:
            # Удаляем спиннер и стиль скелета
            btn.set_child(None)
            btn.remove_css_class("skeleton")
            
            width, height = self.get_thumbnail_size()
            
            # Используем Texture.new_for_pixbuf (так как это единственное, что стабильно работает в PyGObject)
    ...
    def create_placeholder_btn(self, full_url):
        ...
        # Сразу подключаем сигнал, чтобы работало даже при долгой загрузке
    ...
    def load_wallpapers(self, query, page):
        ...
        def worker():
            ...
            try:
                ...
                # --- ГЛАВНОЕ ИЗМЕНЕНИЕ: Сначала создаем UI, потом грузим ---
                # Мы собираем данные для UI в список, чтобы передать в idle_add
                items_to_add = []
                for w in data:
                    thumbs = w.get("thumbs", {})
                ...
                # Передаем список в UI поток для создания заглушек
    ...
    def create_placeholders_and_load(self, items):
        """Создает заглушки в UI потоке и запускает загрузку для каждой."""
        for thumb_url, full_url in items:
            # 1. Создаем и добавляем кнопку-заглушку
            btn = self.create_placeholder_btn(full_url)
            self.flowbox.append(btn)
            
            # 2. Запускаем загрузку, передавая ЭТУ ЖЕ кнопку
...
class FullImageWindow(Gtk.Window):
    ...
    def on_save_finish(self, d, res):
        try:
            f = d.save_finish(res)
            if f:
                with open(f.get_path(), "wb") as file: file.write(self.image_data)
        except Exception: pass
